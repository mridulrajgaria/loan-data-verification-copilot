...
- **Bug fix — cross-source-conflict and document-custody rules could never trigger in the live app**: `runBatchValidation` was always built to accept `servicerUpdates` and `documentManifests` alongside the loan tape (the standalone test scripts already exercise this), but the live `POST /api/uploads` endpoint only ever accepted a single loan-tape file and called validation with both arrays empty — so `RULE_CROSS_SOURCE_CONFLICT` and `RULE_REQUIRED_DOCUMENT_STATUS`'s manifest check were unreachable outside test scripts, no matter how messy the secondary data was. The upload endpoint now accepts two additional optional CSV files (servicer updates, document manifest) and passes them through to validation; the Operator dashboard's upload panel gained two optional file pickers for them.
